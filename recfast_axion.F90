...
module Recombination
  ...
  !!integer,  parameter :: Nz=20000 !RL 112123
    ...
    if (z.ge.zrec(1)) then
       ...
       if (z.le.zrec(nz)) then
          ...
          Recombination_xe=az*xrec(ilo)+bz*xrec(ihi)+ &
               ((az**3-az)*dxrec(ilo)+(bz**3-bz)*dxrec(ihi))/6._dl !original
          !!Recombination_xe=az*xrec(ilo)+bz*xrec(ihi) !RL trying linear interpolation
       endif
    endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
    ...
    n = Nnow * (1._dl+z)**3
           fnu = (21.d0/8.d0)*(4.d0/11.d0)**(4.d0/3.d0) !RL check 031824
    ...
    !!!write(*, *) 'Rayne, z_eq estimation', (3.d0*(HO*C)**2/(8.d0*Pi*G*a_rad*(1.d0+fnu)*Tnow**4))*(OmegaB+OmegaC)
    z_eq=aeq**(-1.0d0)
    z_eq = z_eq - 1.d0
    !!!write(*, *) 'Rayne, actual z_eq', z_eq
    !print*,z_eq
    ...
    do i = 1,Nz
       ...
       if (zend > 8000._dl) then
          ...
          call recdverk(Recomb,OmegaAx,omegar,aeq,aosc,3,ION,zstart,y,zend,tol,ind,cw,nw,w)
          y(1) = x_H0
          x0 = y(1) + fHe*y(2)
          y(4)=y(3)
       else

          call recdverk(Recomb,OmegaAx,omegar,aeq,aosc,nw,ION,zstart,y,zend,tol,ind,cw,nw,w)
    ...
    !!    write(11162302, '(36E52.42E3)') zend, Trad, Tmat, x_H, x_He, x
    end do
    ...
    deallocate(w)
    !!do i = 1, Nz, 1 !RL 122123
    !!   write(12212311, '(36E52.42E3)') zrec(i), xrec(i), dxrec(i)
    !!end do
  ...
  subroutine ION(Recomb,OmegaAx,omegar,aeq,aosc,Ndim,z,Y,f) !RL added aosc after aeq since we can't sue ModelParams here 121524
    use RECDATA
    !use LambdaGeneral !RL 07/31/2023 - can't use
    !       use ModelParams !RL - can't use this because of conflict
    implicit none
    ...
    real(dl) dtauda, grhoax_frac !RL 07/31/2023
    real (dl) gr,sfac,aosc!,rhorefp_hsq
    real(dl) C10, dHdz
    real (dl) OmegaAx,aeq,dorp,dorpa,deriv_eps !!loga_table(ntable),grhoax_table(ntable),grhoax_table_buff(ntable)
    external dtauda, grhoax_frac !RL 07/31/2023
    integer i
    !!real (dl) wcorr_coeff, H_eV !RL
    !!H_eV=1.d14*6.5821d-27*dble(CP%H0)/(MPC_in_sec*c) !RL
...
!!!! as well as of course ULAS
       sfac=1.0d0/(1.0d0+z)
       !RL replacing these code below to using EFA in grhoax_frac(a)----------07/31/2023
       !!       if (sfac .le. aosc) then !RL changed lt to le
       ...
       !!          endif
       !!       else
       !!          dorp=rhorefp_hsq*((aosc/sfac)**3.0d0)
       !RL incorporated the EFA version
       !RL adding w correction to the background
       !wcorr_coeff = CP%ah_osc*aosc/((CP%ma/H_eV)*(CP%H0/100.0d0))
       !dorp=rhorefp_hsq*((aosc/sfac)**3.0d0)*dexp((wcorr_coeff**2.0d0)*9.0d0*(1.0d0/(sfac**4.0d0) - 1.0d0/(aosc**4.0d0))/8.0d0) 
       !!        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       !!deriv_eps=1.d-2*real(sfac) !RL 122123
       !RL replacing these code below to using EFA in grhoax_frac(a)----------07/31/2023
       ...
       !!          endif
       !!       else
       !!          dorpa=rhorefp_hsq*((aosc/(sfac+deriv_eps))**3.0d0)
       !!       endif
       !RL 07/31/2023
...
!!!!! Include self consistently in dH/dz
    ...
    !!write(11162303, '(36e52.42E3)') z, dorp, Hz, dHdz, y(1), f(1), y(2), f(2), y(3), f(3) !RL
  end subroutine ION
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine recdverk (EV,OmegaAx,omegar,aeq,aosc,n, fcn, x, y, xend, tol, ind, c, nw, w)
    use Precision
    use AMLUtils
    ...
    !!real(dl) loga_table(ntable),grhoax_table(ntable),grhoax_table_buff(ntable) !RL 07312023
    real(dl) OmegaAx,omegar,aeq,aosc!!rhorefp_hsq
...
100 continue
    ...
    call fcn(EV,OmegaAx,omegar,aeq,aosc,n, x, y, w(1,1))
    c(24) = c(24) + 1._dl
105 continue
    !
...
200                   continue
                      call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18)/6._dl, w(1,9), w(1,2))
                      !
                      do 205 k = 1, n
                         w(k,9) = y(k) + temp*(   w(k,1)*74569017600._dl &
                              + w(k,2)*298276070400._dl  )
205                      continue
                         call fcn(EV,real(OmegaAx),omegar,aeq,aosc, n, x + c(18)*(4._dl/15._dl), w(1,9), w(1,3))
                         !
                         do 210 k = 1, n
...
210                         continue
                            call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18)*(2._dl/3._dl), w(1,9), w(1,4))
                            !
                            do 215 k = 1, n
...
215                            continue
                               call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18)*(5._dl/6._dl), w(1,9), w(1,5))
                               !
                               do 220 k = 1, n
...
220                               continue
                                  call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18), w(1,9), w(1,6))
                                  !
                                  do 225 k = 1, n
...
225                                  continue
                                     call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18)/15._dl, w(1,9), w(1,7))
                                     !
                                     do 230 k = 1, n
...
230                                     continue
                                        call fcn(EV,real(OmegaAx),omegar,aeq,aosc,n, x + c(18), w(1,9), w(1,8))
                                        !
